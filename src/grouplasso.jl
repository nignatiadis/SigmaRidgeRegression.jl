Base.@kwdef mutable struct GroupLassoRegressor{G, P, T<:Number} <: AbstractGroupRidgeRegressor
...
function MMI.fit(m::GroupLassoRegressor, verb::Int, X, y)
    ...
    p = size(Xmatrix, 2)
	m_tmp = MultiGroupRidgeRegressor(m.groups; decomposition = decomposition, scale = scale, center = center)
